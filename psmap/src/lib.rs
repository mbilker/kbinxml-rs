  ...
  fn value_for_pair(
    ...
    let map_value = match default_value {
      ...
      None => quote_spanned! {source.span()=>
        .with_context(|| {
          format!("Node field `{}` does not have a value in the node", stringify!(#source))
        })?
      },
    };
    ...
    let field_tokens = match default_value {
      ...
      None => quote_spanned! {target.span()=>
        #target: #target.with_context(|| {
          format!("Field `{}` not found for `{}`", stringify!(#target), stringify!(#struct_name))
        })?,
      },
    };
    ...
    if let Some(attributes) = attributes {
      ...
      for SourceMapping { source: attr, target, target_type } in attributes.iter() {
        ...
        body.append_all(quote_spanned! {attr.span()=>
          #target = Some(child.attr(#attr).with_context(|| {
            format!("Attribute `{}` missing from `{}`", #attr, #source)
          })?
          .parse#target_type()
          .with_context(|| {
            format!("Failed to parse `{}` attribute", #attr)
          })?);
        });
        self.fields.append_all(quote_spanned! {target.span()=>
          #target: #target.with_context(|| {
            format!("Field `{}` not found in `{}` for {}",
              stringify!(#target),
              #source,
